...
function magnum-auth {
  ...
  openstack user create --domain default --password cloud magnum
...
# openstack endpoint create --region RegionOne container-infra public http://${controllerIP}:9511/v1
# openstack endpoint create --region RegionOne container-infra internal http://${controllerIP}:9511/v1
# openstack endpoint create --region RegionOne container-infra admin http://${controllerIP}:9511/v1
  ...
  openstack domain create --description 'Container domain owns users and projects created by Magnum' magnum
  openstack user create --domain magnum --password cloud magnum_domain_admin
...
# https://docs.openstack.org/magnum/ocata/troubleshooting-guide.html#heat-stacks
  openstack user create --domain magnum --password cloud trustee_domain_admin
  openstack role add --domain magnum --user-domain magnum --user trustee_domain_admin admin
