...
vmpath=`pwd`/VMs
pike=pike-allin1-centos7
vdisk=${vmpath}/vdbCentOS-7-CinderLVM.qcow2v3

function netreset {
  virsh net-destroy default ; virsh net-undefine default
  virsh net-list
  virsh net-define ../assets/default.xml
...
function vreset {
  ...
  virsh destroy ${pike}-ram${vram}-cpu${vcpu} ; virsh undefine ${pike}-ram${vram}-cpu${vcpu}
...
#   return
  ...
  if [ $? != 0 ] ; then
    echo default libvirtd linux bridge not configured ... 
#   virsh net-create ../assets/default.xml >& /dev/null
    virsh net-define ../assets/default.xml 
    virsh net-autostart default
    virsh net-start default
    virsh net-list
  ...
  echo create 3 vNICs eth0-2 attached to linux bridge virbr0 ... 
...
function virt-xml {
  virsh create ../assets/pike-ovs-centos7-vdb64g-ram16384-cpu8.xml
}
